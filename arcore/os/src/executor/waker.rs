    ...
    F2: Future<Output = T2>,
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if ret.is_ready() {
            if let Poll::Ready(value) = ret {
                return Poll::Ready(SelectOutput::Output1(value));
            } else {
                return Poll::Pending; // Equivalent to `?` returning early for `Poll::Pending`
            }
        }
        ...
        if ret.is_ready() {
            if let Poll::Ready(value) = ret {
                return Poll::Ready(SelectOutput::Output2(value));
            } else {
                return Poll::Pending; // Equivalent to `?` returning early for `Poll::Pending`
            }
        }
