...
lazy_static! {
    ...
    pub static ref INITPROC: Arc<ProcessControlBlock> = {
        let inode = open_file("ch7b_user_shell", OpenFlags::RDONLY).unwrap();
...
#[allow(warnings)]
///把两态切换放进async函数中
pub async fn taskloop(tbc: Arc<TaskControlBlock>) {
    tbc.set_waker(waker::take_waker().await);
    trace!(
        "into task loop"
    );
